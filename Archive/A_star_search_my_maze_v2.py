...
class Cell(object):
    def __init__(self, x, y, cell_walls, visited):
        """
        Initialize new cell
...
class AStar(object):
    def __init__(self, maze_dim, current_maze, locations_visited, current_location):
        self.opened = []
    ...
    def init_grid(self):
        for x in range(self.grid_width):
            for y in range(self.grid_height):
                ...
                if self.locations_visited[y,x] > 0:
                    visited = True
                else:
                    visited = False
                self.cells.append(Cell(x, y, cell_walls, visited))
        self.start = self.get_cell(self.current_location[0], self.current_location[1])
        self.end = self.get_cell(self.grid_height//2, self.grid_width//2)
    ...
    def process(self):
        ...
        while len(self.opened):
            ...
            for adj_cell, order in zip(adj_cells, adj_cells_order):
                ...
                if cell.visited and cell.is_permissible(order) and adj_cell not in self.closed:
                    #input()
...
if __name__ == "__main__":
    filename = r"C:\Users\ajkra\Documents\Courses\DataScientist\Capstone Project\AI_startercode\test1.txt"
    current_location = [0,0]
    testmaze = Maze( filename )
    search = AStar(testmaze.dim, testmaze.walls, current_location)
    search.init_grid()
    search.process()